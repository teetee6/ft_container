...
int main() {
	...
	ft::vector< int > ft_fill(4, 77777);
	ft::vector< int > ft_range(arr, arr + (sizeof(arr) / sizeof(int)));
	ft::vector< int > ft_copy(ft_range);
	std::cout << "----------------------------------------" << std::endl;
	{
		ft::vector<int>::iterator ft_fill_begin = ft_fill.begin();
		ft::vector<int>::iterator ft_fill_end = ft_fill.end();
		std::cout << ft_fill.size() << ", " << ft_fill.capacity() << std::endl;
		for(ft::vector<int>::iterator iter = ft_fill_begin; iter != ft_fill_end; ++iter) {
			std::cout << *iter << " ";
		}
		std::cout << std::endl;	
	}
	{
		ft::vector<int>::iterator ft_range_begin = ft_range.begin();
		ft::vector<int>::iterator ft_range_end = ft_range.end();
		std::cout << ft_range.size() << ", " << ft_range.capacity() << std::endl;
		for(ft::vector<int>::iterator iter = ft_range_begin; iter != ft_range_end; ++iter) {
			std::cout << *iter << " ";
		}
		std::cout << std::endl;	
		{
			ft::vector<int>::iterator ft_copy_begin = ft_copy.begin();
			ft::vector<int>::iterator ft_copy_end = ft_copy.end();
			std::cout << ft_copy.size() << ", " << ft_copy.capacity() << std::endl;
			for(ft::vector<int>::iterator iter = ft_copy_begin; iter != ft_copy_end; ++iter) {
				std::cout << *iter << " ";
			}
			std::cout << std::endl;	
		}
